...
description: >
  ...
  支持将自然语言自动转化为结构化专业提示词，按「风格+艺术家+光影+色彩+细节+主题+创意+个性签名」
  八大模块输出。支持风格双融合、艺术家双融合，适配 Midjourney、Stable Diffusion、DALL·E 等主流平台。
...
4. 自动推荐并补全光影、色彩、细节、创意等维度
5. 适配 Midjourney / Stable Diffusion / DALL·E 等主流平台
6. **自动生图**：API Key 内置在技能目录中，无需额外配置，开箱即用
...
两个版本共享同一主题，但风格/艺术家/光影组合不同。

### Step 6: 自动生图（调用 Agnes AI API）

在输出 prompt 后，自动调用生图脚本生成图片。

#### 6.1 生图脚本

技能内置了 `scripts/agnes_image.py`（主脚本）和 `scripts/agnes_txt2img.py`（兼容包装），封装了 Agnes AI 图像生成 API。

- **API 配置**：**API Key 已内置在技能 `scripts/agnes_config.yaml` 中，无需手动配置**
...
**用版本 A（经典版）的 prompt 生成 4 张图**，同一 prompt 多次采样可获得不同细节变体。
...
**文生图（默认）**：
```bash
cd <workspace>/tech-knowledge-card
...
```bash
cd <workspace>/tech-knowledge-card
echo "style transfer prompt" | python .codebuddy/skills/ai-prompt-master/scripts/agnes_image.py <OUTPUT_PATH> --image <REFERENCE_IMAGE> --strength 0.7
```

**🆕 图生图（批量变体）**：
```bash
cd <workspace>/tech-knowledge-card
printf "prompt\nprompt\nprompt\nprompt" | python .codebuddy/skills/ai-prompt-master/scripts/agnes_image.py <OUTPUT_DIR> --image <REF_IMG> --strength 0.65 --batch --count 4
```

#### 6.5 输出路径

图片保存到 `generated-images/` 目录：
```
...
在完成输出和生图后，执行自增长协议（详见「风格库自增长协议」章节）：

1. 提取本次两个版本中使用的全部风格、艺术家、光影、色彩、细节、创意关键词
...
🖼️ 生成图片：{图片路径，如 generated-images/ai-prompt_20260705_1928.png}
🌱 本次词库增长：{新增条目摘要}
═══════════════════════════════════
```
...
每次输出后，逐项比对：
1. 提取本次两个版本中使用的全部风格名、艺术家名、光影关键词、色彩方案、细节词、创意手法
...
1. **结构优先**：始终按 8 模块结构组织，不遗漏
2. **风格融合**：每次必融合 2 种风格 + 2 位艺术家，形成化学反应
3. **英文为主**：完整 prompt 用英文（主流工具最优），结构拆解用中文
4. **精准匹配**：风格/艺术家/光影必须与主题契合，不可随意拼凑
5. **平台适配**：根据用户指定的平台调整参数格式
6. **双版本策略**：经典版保底 + 创意版出彩
7. **可解释性**：每个选择都要有理由，让用户理解为什么这么搭配
8. **自增长**：每次使用后自动比对词库，追加新发现条目到 `references/growth_log.md`
9. **自动生图**：API Key 内置，输出 prompt 后自动调用 `scripts/agnes_image.py` 生成版本 A 的图片
10. **🆕 图生图支持**：用户提供参考图时自动切换 img2img 模式，`--image` 传入参考图，`--strength` 控制重绘强度
